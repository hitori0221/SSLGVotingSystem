...
<aside id="separator-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform duration-500 ease-in-out -translate-x-full sm:translate-x-0" aria-label="Sidebar">
    <div class="overflow-y-auto bg-zinc-100 dark:bg-neutral-950 relative h-full">
        ...
        <?php if ($school !== null): ?>
            <div  aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom" class="flex items-center justify-between gap-2 mx-4 my-5  hover:bg-neutral-800 rounded cursor-pointer p-2">
        ...
        .then(data => {
            ...
            if (newContent) {
                ...
                newContent.querySelectorAll("script").forEach(script => {
                    ...
                    if (script.src) {
                        newScript.src = script.src; // Re-add external script
                    } else {
                        newScript.textContent = script.textContent; // Re-run inline scripts
                    }
